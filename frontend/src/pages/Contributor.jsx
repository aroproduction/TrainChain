...
export default function ContributorJobs() {
  ...
  const sortedJobs = [...filteredJobs].sort((a, b) => {
    ...
    if (sortBy === "highest-reward") return Number.parseFloat(b.reward || 0) - Number.parseFloat(a.reward || 0)
    if (sortBy === "lowest-reward") return Number.parseFloat(a.reward || 0) - Number.parseFloat(b.reward || 0)
    return 0
  })
  ...
  return (
    <>
      <Navbar />
      <div className="relative min-h-screen flex flex-col items-center py-20 px-4 sm:px-8 md:px-12 lg:px-20 overflow-hidden bg-fixed">
        {/* Enhanced Background */}
        <div className="fixed inset-0 z-[-1] bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 overflow-hidden">
          ...
          <div className="absolute -top-32 -left-16 w-96 h-96 bg-emerald-400 opacity-15 blur-[120px] rounded-full"></div>
          <div
            className="absolute bottom-10 -right-20 w-[500px] h-[500px] bg-green-400 opacity-15 blur-[120px] rounded-full"
          ></div>
          <div
            className="absolute top-1/4 -left-10 w-80 h-80 bg-amber-600 opacity-15 blur-[120px] rounded-full"
          ></div>
          <div
            className="absolute top-1/3 right-10 w-72 h-72 bg-yellow-400 opacity-15 blur-[120px] rounded-full"
          ></div>
        </div>
        ...
        {selectedJob ? (
          ...
          <div className="max-w-7xl w-full py-8 mx-auto">
            ...
            >
              {/* Glass-like overlay with subtle pattern */}
              ...
              {/* Animated lights */}
              <div className="absolute -top-24 -left-24 w-48 h-48 bg-green-300 opacity-40 rounded-full blur-3xl"></div>
              <div className="absolute -bottom-32 -right-32 w-64 h-64 bg-green-400 opacity-30 rounded-full blur-3xl"></div>
                ...
                <div className="flex flex-col md:flex-row items-center justify-between">
                  <div className="mb-8 md:mb-0 md:mr-8 max-w-2xl">
                    <h1 className="text-4xl md:text-5xl font-extrabold mb-4 leading-tight tracking-tight">
                      Contribute Your <span className="text-yellow-300 inline-block relative">
                        Computing Power
                        <span className="absolute bottom-1 left-0 w-full h-1 bg-yellow-400/50 rounded-full"></span>
                      </span>
                    </h1>
                    <p className="text-lg md:text-xl opacity-90 mb-8 leading-relaxed">
                      Join our decentralized network of contributors and earn rewards by training AI models with your
                      GPU.
                    </p>
                    <motion.a
                  ...
                  >
                    <div className="relative">
                      <div className="absolute inset-0 bg-gradient-to-r from-green-500 to-green-700 rounded-full opacity-30 blur-xl"></div>
                      <motion.div
                        animate={{
                          rotate: 360,
                          transition: {
                        ...
                        className="absolute inset-0 rounded-full bg-gradient-to-r from-green-400/20 to-green-600/20"
                        style={{
                          backgroundImage: 'radial-gradient(circle at center, transparent 40%, rgba(16, 185, 129, 0.4) 70%, transparent 75%)'
                        }}
                      ></motion.div>
              ...
              <div className="absolute bottom-0 left-0 right-0 h-16 bg-gradient-to-t from-green-800/70 to-transparent"></div>
            </motion.div>
            ...
            <motion.h1
              ...
              className="text-5xl font-extrabold text-center mt-28 mb-16 text-gray-800 font-serif"
            >
              Available Jobs
            </motion.h1>
                        ...
                        <div className="space-y-2">
                          {["all", "pending", "in_progress", "completed"].map((status) => (
                            <button
                              ...
                              className={`block w-full text-left px-3 py-2 rounded-lg transition-colors duration-200 ${filter === status ? "bg-blue-100 text-blue-700" : "hover:bg-gray-100"}`}
                            >
                        ...
                        <div className="space-y-2">
                          {[
                            ...
                            <button
                              ...
                              className={`block w-full text-left px-3 py-2 rounded-lg transition-colors duration-200 ${sortBy === option.id ? "bg-blue-100 text-blue-700" : "hover:bg-gray-100"}`}
                            >
                              {option.label}
                            </button>
                          ))}
            ...
            {isLoading ? (
              ...
              <AnimatePresence>
                <motion.div
                  ...
                  style={{ perspective: '1000px' }}  // added perspective
                  initial="hidden"
                  animate="visible"
                  variants={{
...
function JobCard({ job, onClick }) {
  ...
  return (
    ...
    >
      ...
      {/* Glow effect overlay */}
      <div
        className={`absolute inset-0 rounded-2xl bg-gradient-to-br from-blue-400/10 via-purple-400/10 to-pink-400/10 transition-opacity duration-300 ${isHovering ? 'opacity-100' : 'opacity-0'}`}
      />

      {/* Colored border effect */}
      <div
        className={`absolute inset-0 rounded-2xl border-2 border-transparent transition-all duration-500 ${isHovering ? 'border-blue-400/50' : ''}`}
        style={{
          backgroundClip: 'padding-box',
          boxShadow: isHovering ? '0 0 15px rgba(59, 130, 246, 0.4)' : 'none'
        }}
      />
